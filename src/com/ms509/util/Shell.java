...
public class Shell {
	...
	private String[] execute_php(String command) {
		...
		assert z12 != null;
		tmp = (Base64.getEncoder().encodeToString(z12)) + "&" + Safe.PARAM2 + "="
				+ (Base64.getEncoder().encodeToString(z22));
	...
	private String[] execute_cus(String command) {
		...
		case 1:
			z1 = "cmd";
			// z2 = "cd" + " " + "/d " + "\"" + pa + "\"&" + command + "&echo
			// [S]&cd&echo [E]";
			break;
		case 2:
			z1 = "/bin/sh";
			// z2 = "cd" + " " + "\"" + pa + "\";" + command + ";echo
			// [S];pwd;echo [E]";
			break;
		default:
			break;
		...
		z2 = z2.replace("\n", "");
		z2 = z2.replace("\r", "");
		// params = Common.makeParams(Safe.CUS_MAKE,
		// Safe.CUS_SHELL,URLEncoder.encode(z1),URLEncoder.encode(z2),URLEncoder.encode(pa));
		try {
			params = Safe.PASS + "=1&" + Safe.ACTION + "=" + Safe.CUS_SHELL + "&" + Safe.PARAM1 + "="
					+ URLEncoder.encode(z1, "") + "&" + Safe.PARAM2 + "=" + URLEncoder.encode(z2, "") + "&"
					+ Safe.PARAM3 + "=" + URLEncoder.encode(pa, "");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
		}
		if (null != params) {
	...
	private String CheckCMDPath(String z) // 检查cmd是否有自定义路径
	{
		if (cus_z1.equals("")) {
			return z;
		} else {
	...
	public String GetPath() {
		...
		try {
			...
			case 3:
				...
				break;
			case 4: // cus模式
				// path_index = Common.makeParams(Safe.CUS_MAKE,
				// Safe.CUS_INDEX);
