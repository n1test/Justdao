...
public class DatabasePanel extends JPanel {
	...
	private JComboBox<Object> choosedb;
	...
	private void setComponent() {
		...
		JLabel selectdb = new JLabel("数据库");
		JLabel sql_list = new JLabel("常用SQL语句");

		// 数据库jtree
		dblist = new JTree();
		JScrollPane dblistpane = new JScrollPane(dblist);
		...
		// 数据库查询结果窗口
		datalist = new JTable();
		JScrollPane datalistpane = new JScrollPane(datalist);
		...
		new DBPopMenu(this, dblist, datalist, status);

		// 命令执行窗口
		commandlist = new JTextPane();
		JScrollPane commandpane = new JScrollPane(commandlist);
		commandlist.setText("");
		...
		// 数据库配置按钮
		JButton dbset = new JButton("数据库配置");
		OpenDBDialog action = new OpenDBDialog();
		dbset.addActionListener(action);

		// sql命令执行
		JButton exec = new JButton("执行SQL(Ctrl+回车)");
		Exec execation = new Exec();
		exec.addActionListener(execation);

		// 初始化布局
		this.setLayout(new GridBagLayout());
		GBC gbcleft = new GBC(0, 0, 2, 4).setFill(GBC.VERTICAL).setWeight(0, 1).setIpad(300, 0);
		...
		public void mouseClicked(MouseEvent e) {
			...
			try {
				...
				for (int m = 0; m < choosedb.getItemCount(); m++) // 选中jtree，设置下拉框中对应项
				{
					...
					} else if (choosedb.getItemAt(m).toString()
							.equals(dblist.getSelectionPath().getPathComponent(1).toString())) {
						choosedb.setSelectedIndex(m);
					}
				}
			} catch (Exception k) {
	...
	private void UpdateData(String result) {
		...
		for (String aDtitle : dtitle) {
			vtitle.add(aDtitle.replace("\t\\|\t", ""));
		}
		if (rows.length > 1) {
