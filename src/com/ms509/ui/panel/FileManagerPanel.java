...
public class FileManagerPanel extends JPanel {
	...
	public FileManagerPanel() {
		...
		GBC gbctree = new GBC(0, 1).setWeight(0, 1).setFill(GBC.VERTICAL).setIpad(300, 0);
		...
		try {
			DbDao.getInstance().getStmt()
					.executeUpdate(("update data set ip='" + ip + "',time='" + time + "' where id=" + id));
		...
		Runnable run = () -> {
			...
			if (arrtmp.contains("HTTP/1.") || !arrtmp.contains("/") && !arrtmp.contains("\\")) {
	...
	private void filemanagerindex() {
		String[] index_datas = arrtmp.split("\t");
	...
	private void filemanagersystem() {
		...
		Runnable run = () -> {
			...
			SwingUtilities.invokeLater(() -> {
				TreeMethod.makeIndexTree(tmp2, trees, TreeMethod.searchNode(root, search), tree);
				...
				if (tp != null) {
					...
					Runnable run = () -> {
						while (true) {
							Thread.yield();
							if (lstatus && rstatus) {
								SwingUtilities.invokeLater(() -> status.setText("完成"));
								break;
							}
						}
					};
