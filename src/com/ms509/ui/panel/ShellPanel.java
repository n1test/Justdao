...
public class ShellPanel extends JPanel {
	...
	public ShellPanel() {
		...
		Thread thread_getpath = new Thread(() -> {
			// 显示网站路径
			path = core.GetPath();
			check_path();
			final String tmp1 = path.substring(0, path.length() - 1);
			SwingUtilities.invokeLater(() -> {
				if (tmp1.contains("HTTP/1.") || !tmp1.contains("/") && !tmp1.contains("\\")) {
					new MessageDialog(tmp1, 5);
		...
		// Font shell_font = new Font("幼圆", Font.BOLD, 12);
				...
				try {
					...
					} else if (tmp_cmd.indexOf("setp") == 0) // 自定义路径
					{
						...
						Thread exe = new Thread(() -> {
							...
							try {
								// Thread.sleep(10000);
								execute(path, shell_doc.getText(command_start, command_stop - command_start), os, type);
							} catch (Exception e) {
	...
	// 命令执行调用
	private void execute(String pa, String command, int os, int type) {
		String result[];
		try {
			path_bak = path;
			core.SetPath(path);
	...
	// 下翻历史命令
	private String key_down_action() {
		num = num + 1;
		String last_command = null;
